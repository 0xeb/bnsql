        ...
        .cache_builder([](std::vector<InsnInfo>& cache) {
            ...
            for (auto& func : bv->GetAnalysisFunctionList()) {
                for (auto& block : func->GetBasicBlocks()) {
                    ...
                    while (addr < end) {
                        size_t len = bv->GetInstructionLength(arch, addr);
                        if (len == 0) {
                            addr++;
                            continue;
                        }
                        ...
                        // Get disassembly tokens via architecture
                        std::vector<InstructionTextToken> tokens;
                        size_t instrLen = 0;
                        DataBuffer buf = bv->ReadBuffer(addr, 16);
                        arch->GetInstructionText(static_cast<const uint8_t*>(buf.GetData()), addr, instrLen, tokens);

                        // Build mnemonic and full disassembly
                        std::string disasm;
                        for (auto& tok : tokens) {
