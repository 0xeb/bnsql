...
/**
 * AIAgent - Natural language interface for BNSQL using libagents
 *
 * This is a wrapper around libagents that provides:
...
public:
    ...
    /**
     * Get the current session ID (for display/logging)
     * @return Session ID string from the provider
     */
...
private:
    ...
    /// Register the bnsql tool with libagents
    void setup_tools();
