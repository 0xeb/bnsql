...
// Agent settings stored in ~/.bnsql/agent_settings.json
...
// Get the settings directory path (~/.bnsql or %APPDATA%\bnsql)
inline std::string GetSettingsDir() {
#ifdef _WIN32
    char path[MAX_PATH];
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_APPDATA, nullptr, 0, path))) {
        return std::string(path) + "\\bnsql";
    }
    // Fallback to USERPROFILE
    const char* userprofile = std::getenv("USERPROFILE");
    if (userprofile) {
        return std::string(userprofile) + "\\.bnsql";
    }
    return ".bnsql";
#else
