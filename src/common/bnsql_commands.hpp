    ...
    std::string& output)
{
    ...
    if (input == ".help" || input == ".h") {
        output = "BNSQL Commands:\n"
                 ...
                 "\n"
                 "AI Agent:\n"
                 "  .agent provider [NAME]  Show/set provider (claude, copilot)\n"
                 "  .agent timeout [MS]     Show/set response timeout\n"
                 "  .agent byok [...]       BYOK settings (key, endpoint, model)\n"
...
#else
        output = "AI agent support not compiled in. Rebuild with -DBNSQL_WITH_AI_AGENT=ON\n";
#endif
        return CommandResult::HANDLED;
    }
...
#else
    return {false, "Error: AI agent not compiled in. Rebuild with -DBNSQL_WITH_AI_AGENT=ON\n", 1};
#endif
}
