...
// Embedded documentation from prompts/bnsql_agent.md
#include "bnsql_agent_prompt.hpp"
...
void AIAgent::setup_tools() {
    libagents::Tool bnsql_tool;
    bnsql_tool.name = "bnsql";
    bnsql_tool.description =
        "Execute a SQL query against a Binary Ninja database. "
        "Available tables: funcs, strings, imports, segments, names, xrefs, instructions, "
        "blocks, comments, entries, db_info. "
        "Example: SELECT name, size FROM funcs WHERE name LIKE 'sub_%' ORDER BY size DESC LIMIT 10";

    bnsql_tool.parameters_schema = R"({
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
    ...
    bnsql_tool.handler = [this](const std::string& args) -> std::string {
        try {
    ...
    agent_->register_tool(bnsql_tool);

    if (verbose_) {
        std::cerr << "[AGENT] Registered bnsql tool" << std::endl;
    }
}
...
std::string AIAgent::build_primed_message(const std::string& user_message) {
    return std::string(SYSTEM_PROMPT) +
        "\n\n---\n\n"
        "# User Request\n\n"
        "Use the `bnsql` tool to execute SQL queries. "
        "Do not use Bash, Grep, or other tools - only use the bnsql tool.\n\n" +
        user_message;
}
...
std::string AIAgent::get_session_id() const {
    if (agent_ && agent_->is_initialized()) {
        return agent_->get_session_id();
    }
    return "";
}
